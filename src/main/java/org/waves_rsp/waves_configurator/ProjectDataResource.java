...
public class ProjectDataResource {
    ...
    public Response loadTriG(String trig) throws InterruptedException {
    	...
    	try {
			...
			e.printStackTrace();
			if( e instanceof JenaTransactionException 	){ errorMessage = "ERROR: Invalid TriG"; }
			else if( e instanceof ZeroGraphException 	){ errorMessage = "ERROR: This TriG contains no graph"; }
			else if( e instanceof MultiGraphsException 	){ errorMessage = "ERROR: This TriG contains more than one graph"; }
			else if( e instanceof HttpException 		){ errorMessage = "ERROR: Can not connect to Triple Store"; }
			else { errorMessage = "Unknown Error: Check the server to debug"; }
		}
   ...
   public Response getProjectInfo(String projectName){
	   ...
	   try {
			...
			e.printStackTrace();
			if( e instanceof ProjectNotExistException ){ errorMessage = "Error: Project doesn't exist"; } 
			else if( e instanceof JSONException		  ){ errorMessage = "Error: Project location cannot be converted to json object"; } 
			else { errorMessage = "ERROR: Can not connect to Triple Store";}
	   }
	   try {
			entity.put("success", success);
			if( !success ){ 
